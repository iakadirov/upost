    ...
    <?= GridView::widget([
        ...
        'rowOptions'=>function ($model, $key, $index, $grid){
            ...
            [
	            ...
	            'content'=>function($data){
	            	if($data->priority == 1){
	            		return '<span data-id="'.$data->id.'" class="label label-danger">'.Yii::t('idev','Important').'</span>';
	            	}else{
	            		return '<span data-id="'.$data->id.'" class="label label-default">'.Yii::t('idev','Plain').'</span>';	            		
	            	}
	            }
		        ],
		        [
		        ...
		        [
	            'label'=>Yii::t('idev','Edit/Delete'),
	            'contentOptions' =>function ($model, $key, $index, $column){
	                return ['class' => 'postActions'];
	            },
	            'content'=>function($data){
	            	return '<a href="'.IdevFunctions::to('/news/edit/'.$data->id).'">'.Yii::t('idev','Edit').'</a><br/><a href="'.IdevFunctions::to('/news/delete/'.$data->id).'">'.Yii::t('idev','Delete').'</a>';
	            }
		        ],
        ],
		]);?>
...
<div id="sortNews" class="modal fade">
	<div class="modal-dialog dsModal">
		<div class="modal-content modal-sm">
			...
			<?php $form = ActiveForm::begin(['id'=>'contentSort']); ?>
					...
					<div class="form-group mb-10">
						...
						<select class="form-control" name="Sort[category]">
							...
							<?php foreach ($data['category'] as $key => $value): ?>
								<option value="<?=$value['category_id']?>" <?=isset($sort['category']) && $sort['category'] == $value['category_id']?'selected':''?> ><?=$value['name']?></option>
							<?php endforeach ?>
						</select>
					</div>
					<div class="form-group mb-10">
						...
						<select class="form-control" name="Sort[author]">
							...
							<?php foreach ($data['user'] as $key => $value): ?>
								<option value="<?=$value['id']?>" <?=isset($sort['author']) && $sort['author'] == $value['id']?'selected':''?>><?=$value['username']?></option>
							<?php endforeach ?>
						</select>
					</div>
					<div class="form-group mb-10 col-sm-6 pull-left pl-0 pr-5">
						<label class="m-0"><?=Yii::t('idev','Data from')?></label>
						<input type="date" <?=isset($sort['data_from']) && $sort['data_from'] != 0?'value="'.date('Y-m-d',$sort['data_from']).'"':''?> name="Sort[data_from]" class="form-control">
					</div>
					<div class="form-group mb-10 col-sm-6 pull-left pr-0 pl-5">
						<label class="m-0"><?=Yii::t('idev','Data to')?></label>
						<input type="date" <?=isset($sort['data_to']) && $sort['data_to'] != 0?'value="'.date('Y-m-d',$sort['data_to']).'"':''?> name="Sort[data_to]" class="form-control">
					</div>
			</div>
			<div class="modal-footer p-10">
				<button class="btn btn-sm btn-primary" type="submit"><i class="icon-sort"></i> <?=Yii::t('idev','Sort')?></button>
				<button class="btn btn-sm btn-danger" id="removeSort"><i class="icon-cross"></i> <?=Yii::t('idev','Delete sort')?></button>
			</div>
			<?php ActiveForm::end(); ?>
		</div>
	</div>
